...
atom
	proc
		...
		left(w, h = 0)
			...
			for(var/atom/a in obounds(src, -w, h_off, -pwidth + w, h - pheight))
				if(a.pz > pz + pdepth) continue
				if(a.pz + a.pdepth < pz) continue
				. += a
		...
		right(w, h = 0)
			...
			for(var/atom/a in obounds(src, pwidth, h_off, -pwidth + w, h - pheight))
				if(a.pz > pz + pdepth) continue
				if(a.pz + a.pdepth < pz) continue
				. += a
		...
		top(h, w = 0)
			...
			for(var/atom/a in obounds(src, w_off, pheight, w - pwidth, -pheight + h))
				if(a.pz > pz + pdepth) continue
				if(a.pz + a.pdepth < pz) continue
				. += a
		...
		bottom(h, w = 0)
			...
			for(var/atom/a in obounds(src, w_off, -h, w - pwidth, -pheight + h))
				if(a.pz > pz + pdepth) continue
				if(a.pz + a.pdepth < pz) continue
				. += a
