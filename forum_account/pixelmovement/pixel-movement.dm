...
// code to handle 2D mode:
		...
		pixel_move(dpx, dpy)
			...
			for(var/atom/t in obounds(src, dpx, dpy, abs(dpx), abs(dpy)))
...
// otherwise, we handle 3D movement
		...
		pixel_move(dpx, dpy, dpz = 0)
			...
			for(var/atom/a in obounds(src, dpx, dpy, abs(dpx), abs(dpy)))
...
// end of 3D pixel_move proc
		...
		set_pos(nx, ny, nz = 0,map_z = -1)
			...
			if(moved)
				...
				var/tx = round((nx + pwidth / 2) / icon_width)
				var/ty = round((ny + pheight / 2) / icon_height)
